...
def main(config: DictConfig) -> None:
    ...
    study_name = f"emg2qwerty_optuna_{config.model._target_.split('.')[-1]}"
    ...
    for key, value in best_trial.params.items():
        ...
        elif key in ["d_model", "nhead", "num_layers", "dim_feedforward", "dropout"] and "transformer" in best_config.model._target_.lower():
            # Add transformer-specific parameters to the model config
            OmegaConf.update(best_config, f"model.{key}", value)
        elif key == "dropout" and "tds_conv_ctc" in best_config.model._target_:
            best_config.model.dropout = value
        elif key == "kernel_size" and "tds_conv_ctc" in best_config.model._target_:
            best_config.model.kernel_size = value
        elif key in ["lstm_hidden_size", "num_layers"] and "lstm" in best_config.model._target_.lower():
            # Add LSTM-specific parameters
            OmegaConf.update(best_config, f"model.{key}", value)
        elif key in ["gru_hidden_size", "num_layers", "dropout"] and "gru" in best_config.model._target_.lower():
            # Add GRU-specific parameters
            OmegaConf.update(best_config, f"model.{key}", value)
        elif key == "warmup_steps":
            best_config.lr_scheduler.warmup_steps = value
