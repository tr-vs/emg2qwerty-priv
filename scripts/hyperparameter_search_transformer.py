...
def objective(trial: optuna.Trial, base_config: DictConfig) -> float:
    ...
    config.module.kernel_width = trial.suggest_categorical("kernel_width", [16, 32])
    
    # Use a fixed set of valid (d_model, nhead) pairs
    model_head_pair = trial.suggest_categorical("model_head_pair", [
        (256, 4),   # 256 / 4 = 64
        (256, 8),   # 256 / 8 = 32
        (512, 4),   # 512 / 4 = 128
        (512, 8),   # 512 / 8 = 64
    ...
    # Unpack the selected pair
    config.module.d_model, config.module.nhead = model_head_pair
    
    config.module.dim_feedforward = trial.suggest_categorical("dim_feedforward", [1024, 2048, 3072])
...
def main(config: DictConfig) -> None:
    ...
    for key, value in best_trial.params.items():
        ...
        elif key in ["block_channels", "kernel_width", "d_model", "nhead", "dim_feedforward"] and "TransformerModule" in best_config.module._target_.lower():
