...
def objective(trial: optuna.Trial, base_config: DictConfig) -> float:
    ...
    # For subsequent trials, try different parameters
    config.optimizer.lr = trial.suggest_categorical("lr", [1e-4, 1e-3, 1e-2])
    config.module.hidden_size = trial.suggest_categorical("hidden_size", [64, 128, 216])
    config.module.num_layers = trial.suggest_int("num_layers", 2, 6)
...
def main(config: DictConfig) -> None:
    ...
    for key, value in best_trial.params.items():
        ...
        elif key in ["hidden_size", "num_layers"] and "LSTMModule" in best_config.module._target_.lower():
